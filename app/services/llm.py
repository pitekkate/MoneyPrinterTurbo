...
def _generate_response(prompt: str) -> str:
    try:
        ...
        if llm_provider == "g4f":
            ...
            if not api_key:
                raise ValueError(
                    f"{llm_provider}: api_key tidak diatur, silakan atur di file config.toml."
                )
            if not model_name:
                raise ValueError(
                    f"{llm_provider}: model_name tidak diatur, silakan atur di file config.toml."
                )
            
            # Gunakan base_url default jika tidak diset
            if not base_url:
            ...
            headers = {
                ...
                "HTTP-Referer": os.environ.get("YOUR_SITE_URL", "https://yourdomain.com"), # Opsional
                "X-Title": os.environ.get("YOUR_APP_NAME", "MoneyPrinterTurbo"), # Opsional
            }
            ...
            data = {
                "model": model_name,
                "messages": [
                    {"role": "system", "content": "You are a helpful assistant for creating engaging video scripts."},
